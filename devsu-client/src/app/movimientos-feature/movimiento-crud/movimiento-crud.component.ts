...
export class MovimientoCrudComponent implements OnInit {
  ...
  guardarMovimiento(): void {
    if (this.validateMovimientoForm()) {
      ...
      if (this.selectedMovimiento && this.selectedMovimiento.movimientoId) {
        this.movimientoService.updateMovimiento(this.selectedMovimiento).subscribe((res: Movimiento) => {
            this.selectedMovimiento = res;
            this.messageservice.sendSuccessMessage('El movimiento ha sido modificado.');
  ...
  deleteMovimiento(): void {
    if (this.selectedMovimiento && this.selectedMovimiento.movimientoId) {
      this.movimientoService.deleteMovimientoById(this.selectedMovimiento.movimientoId).subscribe(() => {
